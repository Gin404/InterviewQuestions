...
7. CallServerInterceptor: 流操作(写出请求体、获得响应数据) 负责向服务器发送请求数据、从服务器读取响应数据 进行http请求报文的封装与请求报文的解析;  
...
https://juejin.cn/post/7033307467199021086#heading-26
...
### Retrofit实例的构建
主要包含4部分：  
1. okhttp3.Call.Factory callFactory: 实际的请求调用，如 okhttp3.OkHttpClient。
2. HttpUrl baseUrl: 基础URL，如：域名。
3. List<Converter.Factory> converterFactories: 数据转换器列表。
4. List<CallAdapter.Factory> callAdapterFactories: 请求适配器列表。

### 构建 API 接口实例
通过动态代理的方式构建请求接口的实例。  
通过InvocationHandler.invoke代理所有方法的调用，如果**不是Object的方法和系统的方法**，就会调用ServiceMethod.parseAnnotations（最终其实是HttpServiceMethod.parseAnnotations）生成一个**HttpServiceMethod**实例，并缓存。  
上面的方法校验可以由代码设置是在create阶段对所有方法统一校验还是调用阶段再校验，相当于是是否懒加载的区别。  
由ServiceMethod.invoke执行请求方法。

### HttpServiceMethod的构建过程
1. 首先获取目标方法级别的所有注解。这里肯定包括@Get、@Post、@Path等等。
2. 获取方法的返回类型。比如常用的Call<T>。
3. 根据返回类型和Annotation实例化一个 CallAdapter 对象，CallAdapter主要作用就是把真实数据类型T适配为Call<T>，通常适配为实现类ExecutorCallbackCall，用于在主线程回调结果。
4. 根据实际返回类型实例化一个Converter对象，将 okhttp3.ResponseBody 转换成 ResponseT 类型。
5. 构建一个RequestFactory，请求的工厂类。

### 调用HttpServiceMethod.invoke返回一个Call
实际上就是用CallAdapter返回了一个ExecutorCallbackCall

### 最后调用Call的execute
1. ExecutorCallbackCall的enqueue和execute最终还是由Okhttp完成的，只不过enqueue帮我们切换为了UI线程。
2. Converter负责将Okhttp返回的response转换为我们期望的类型。

### 为什么要引入CallAdapter与Converter?
1. Retrofit不光光只支持Call，他还可以将请求适配成Observable类型，方便与RxJava2结合起来一起使用。这就是通过CallAdapter来进行适配工作的，例如通过默认的DefaultCallAdapterFactory将请求转换成Call<Object>，通过RxJava2CallAdapter将请求转换成Observable<Object>。
2. 大部分业务情况下，我们在拿到响应体后都会将其进行反序列化成对象，方便调用。显然，Retrofit就考虑到了这一点，所以他默认提供了GsonConverterFactory，来帮助我们做这一步反序列化工作。这就是通过Converter来完成的，同时它也支持用户进行自定义。

### 用到的设计模式
1. 动态代理模式
为啥用？  
Retrofit按照**RESTful**风格设计并通过注解来定义API方法的请求参数，并将这些API方法放到interface中。因为interface是不能被实例化的，所以这里采用动态代理在运行期间实例化API接口，获取到方法的请求参数。
**解耦**，将实际业务与Retrofit隔离开来。用户只需通过注解方法来定义请求参数，而实际请求的构建则通过Retrofit内部来实现。
2. 适配器模式
比如CallAdapter对Call进行转换。
3. 工厂方法模式
比如ConverterFactory生产Converter。
4. 建造者模式
比如Retrofit的构建。  

参考：https://juejin.cn/post/7044072244451999757
