...
**概念**：在程序运行时，程序有能力获取一个类的所有方法和属性；并且对于任意一个对象，可以调用它的任意方法或者获取其属性。java文件需要编译成.class文件才能被jvm加载使用,对象的.class数据在jvm里就是Class<T>；我们如果能拿到这个Class<T>对象，就能获取该Class<T>对应的对象类型，及在该类型声明的方法和属性值；还可以根据Class<T>创建相应的类型对象，通过Field,Method反过来操作对象。**应用**：
...
5. Java中一个重要应用场景是动态代理。
   **反射创建一个对象**
   主要是如何获取到Class对象。
6. 通过实例获取：object.getClass()
7. 通过已知类型：Object.class
8. 通过Class.forName获取全路径指定类名的class，最终会调用到native方法。
...
GC的两个工作：内存的划分和分配；垃圾回收。内存分配：粗一点儿分为新生代、老年代。新生代细分为Eden、From Survivor、To Survivor。垃圾回收的对象：不被引用的对象，GC需要**标记**这些对象。**标记算法**引用计数法（主流不使用）：每个对象都有个引用计数器，如果被引用就+1，为0时就被回收。缺点：不能解决循环依赖的问题，比如A和B互相引用，再无其他引用，实则应该被回收。根搜索算法（主流在使用）：选定一些对象作为GC roots。以这些对象为起点，构成一个引用树，不在这里的对象说明没有被引用（不可达），可以被回收。作为GC root的对象包括：Java栈中引用的对象，本地方法栈中引用的对象、方法区运行时常量池引用的对象、方法区静态属性引用的对象、运行中的线程、Boorstrap ClassLoader记载的对象、GC控制的对象。**标记后的对象会被立即回收吗？**不会，如果对象不可达，并且GC准备好重新分配空间后，会先执行对象的**finalize**方法。如果finalize后仍然不可达，则会真正被回收。**垃圾收集算法**
...
## Android的Dalvik或ART虚拟机 与 标准JVM

**1. 执行环境与字节码**

* **标准 JVM (如HotSpot)**：执行 **Java 字节码**（存储在 `.class` 文件中）。一个 `.java` 文件编译后会生成一个或多个 `.class` 文件，最后打包成 `.jar`。JVM加载这些文件来运行程序 。
* **Android 虚拟机 (DVM/ART)**：执行 **Dalvik 字节码**（存储在 `.dex` 文件中）。在Android开发中，`.java` 文件先被编译成 `.class` 文件，然后 Android SDK 中的 `dx` 或 `d8` 工具会将所有 `.class` 文件转换成一个或多个 `classes.dex` 文件，最后打包进 `.apk` 。

**关键优势**：`.dex` 文件非常巧妙，它会将多个 `.class` 文件中的常量池等信息进行去重和整合，形成**一个全局的常量池** 。这样做极大地减少了应用安装包（APK）的体积，也节省了运行时的内存，非常适合资源受限的移动设备。

**2. 虚拟机架构**

* **标准 JVM**：**基于栈架构** 。这意味着几乎所有的指令操作都需要通过栈来完成。例如，执行 `a + b`，需要先把 `a` 和 `b` 压入栈，然后执行加法指令，最后将结果弹出栈。这种设计使得指令数较多，但代码体积小，易于在不同硬件上移植。
* **Android 虚拟机 (DVM/ART)**：**基于寄存器架构** 。指令直接操作CPU的寄存器。由于显式指定了操作数，指令通常更短，执行速度也更快。这对于CPU性能相对较弱的移动设备来说，是更优的选择。

> 为了让你直观感受，可以参考一个加法运算的例子：
> **JVM 字节码**：需要 `iload_1`, `iload_2`, `iadd`, `istore_3` 等多条指令 。
> **Dalvik 字节码**：可能只需 `add-int v0, v1, v2` 一条指令 。

**3. 编译与执行策略**

这是ART虚拟机对Dalvik最重大的革新，也是面试中非常喜欢问的区别。

* **Dalvik 虚拟机**：主要依赖 **JIT（即时编译）** 。应用运行时，Dalvik会通过解释器执行dex字节码。同时，它会分析并找出“热点代码”（如频繁执行的方法），然后通过JIT将这些热点代码编译成本地机器码以提高执行效率。但每次应用重新启动，这个过程都要重来一遍。
* **ART 虚拟机**：结合了 **AOT（预编译）和JIT** 。
  * **Android 5.0 \~ 6.0**：ART 采用 **AOT**。应用在**安装时**，dex字节码就会被彻底编译成本地机器码。这使得应用启动和运行速度极快，但代价是安装时间长、占用存储空间大。
  * **Android 7.0+**：ART 采用了混合模式。应用安装时不做编译，先快速安装；首次运行时，通过解释器执行，同时JIT对热点代码进行编译；当设备空闲和充电时，后台进程会把这些编译好的机器码“转储”成AOT文件。这种策略兼顾了**安装速度、启动速度和系统性能**的平衡。

**4. 内存管理与进程模型**

* **内存划分**：Android虚拟机（DVM/ART）的内存区域划分（如 Java 堆、栈、方法区）与标准JVM的运行时数据区在逻辑上是**高度一致的** 。它们都包含程序计数器、栈、堆、方法区等概念。因此，我们之前聊的JVM内存模型知识，在Android中同样适用。
* **进程模型**：标准JVM应用通常是**一个进程启动一个JVM**。Android则是 **“一个应用，一个进程，一个虚拟机实例”** 。而且，所有的应用进程都是由一个叫 **Zygote** 的孵化器进程 **fork** 出来的 。Zygote在系统启动时就已经创建并加载了所有核心框架类和资源，这样 fork 新进程时就非常快，并且所有应用进程能和Zygote**共享一部分内存**（主要是系统资源部分），极大地节省了内存开销。

**5. 类加载机制**

* **相同点**：两者都遵循 **双亲委派模型** 。
* **不同点**：加载的“物料”和实现类不同。
  * **标准 JVM**：从 `.class` 或 `.jar` 文件中加载类。主要的类加载器有 `BootstrapClassLoader`、`ExtClassLoader`、`AppClassLoader` 。
  * **Android 虚拟机**：从 `.dex` 文件中加载类。类加载器的继承体系也类似，但实现类主要是 `PathClassLoader` 和 `DexClassLoader`，它们都继承自 `BaseDexClassLoader` 。
    * `PathClassLoader`：用于加载已安装到系统中的APK文件（即 `/data/app` 目录下的dex）。
    * `DexClassLoader`：可以从外部的 `.apk`、`.jar` 或 `.dex` 文件中加载类，**支持从SD卡等外部存储加载**，是实现插件化和热修复的核心基础 。

## JMM内存模型

硬件层内存模型：cpu寄存器（线程私有） -> 高速缓存（线程私有） -> 主内存RAM（线程共享）JMM内存模型：
...
volatile关键字为实例域的同步访问提供了免锁机制。volitale保证可见性（一个线程对值的修改立刻对其他线程可见）和有序性（禁止指令重排序，操作volitale变量之前的操作肯定在之后的操作前完成了），但是不保证原子性（比如自增操作）。volitale某些情况下能替代syncronized，但是涉及到自增、自减，或者变量包含在其他变量的不变式中就不能用这个关键字（setUpper setLower的例子）。**用处**
...
volatile前面说过了，某些情况可以代替锁。ReentrantLock和synchronized都是悲观锁。二者区别如下：
